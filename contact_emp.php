...
if (isset($_POST['objet']) && isset($_POST['message'])) {
 if (!empty($_POST)) {
  ...
  if (sizeof($e) < 1) {
   $rcc    = "INSERT into contact(Emeteur,destinataire,objet,message,REFPERSONNE) values ('" . $_SESSION['Email_Emp'] . "','Admin','" . $objet . "','" . $message . "'," . $_SESSION['ID_EMPLOYE'] . ")";
...
if (!empty($_GET)) {
 if ($_GET['f'] == "message_reçus") {
  $perpage    = 7;
  $cpage      = 1;
  $req5       = "SELECT count(*) as Nb FROM contact Where Emeteur='Admin' and Destinataire='" . $_SESSION['npe'] . "'";
  ...
  $req4 = "SELECT * FROM contact Where Emeteur='Admin' and Destinataire='" . $_SESSION['npe'] . "' LIMIT " . (($cpage - 1) * $perpage) . ", " . $perpage;
  $res4 = $conn->query($req4);
 }
 if ($_GET['f'] == "message_envoyés") {
  $perpage    = 7;
  $cpage      = 1;
  $z1         = "select count(*) as nbmesenv from contact where Emeteur='" . $_SESSION['Email_Emp'] . "'";
  $rz1        = $conn->query($z1);
  ...
  $req4 = "SELECT * FROM contact Where Emeteur='" . $_SESSION['Email_Emp'] . "' LIMIT " . (($cpage - 1) * $perpage) . ", " . $perpage;
  $res4 = $conn->query($req4);
 }
}
?>
...
<div id="content" class="contact">
  <div id="pager">
    <a href="Contact_Emp.php" class="btn btn-primary">Envoyer Un Message</a> <a
      href="Contact_Emp.php?f=message_reçus&p=1" class="btn btn-info">Message Reçus</a>
  </div>
  <hr>
  <div class="content-wrapper contcli">
    ...
    <table class="table table-hover proj2 rec">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><a href="Lire.Php?f=message_envoyés&message=<?php echo $r['ID'] . "&p=" . $cpage . "&s=emp"; ?>"
    ...
    <nav>
      <ul class="pagination">
        <li <?php if (isset($_GET['p'])) {if ($_GET['p'] == 1) {echo "class='disabled'";}} ?>><a
            href="Contact_Emp.php?f=message_envoyés&p=<?php if (isset($_GET['p'])) {if ($_GET['p'] >= 2) {echo ($_GET['p'] - 1);} else {echo "1";}} ?>"><span>&laquo;</span></a>
        </li>
        <?php for ($i = max(1, $cpage - 4); $i <= max(5, $cpage); $i++) { ?>
        <li <?php if (isset($_GET['p'])) {if ($_GET['p'] == $i) {echo "class='active'";}} ?>><a
            href="Contact_Emp.php?f=message_envoyés&p=<?php echo $i; ?>"><?php echo $i; ?></a></li>
        <?php } ?>
        <li <?php if (isset($_GET['p'])) {if ($_GET['p'] == $nbrpages) {echo "class='disabled'";}} ?>><a
            href="Contact_Emp.php?f=message_envoyés&p=<?php if (isset($_GET['p'])) {if ($_GET['p'] <= $nbrpage - 1) {echo ($_GET['p'] + 1);} else {echo $nbrpages;}} ?>"><span>&raquo;</span></a>
        </li>
      </ul>
    </nav>
    <?php } ?>
    ...
    <nav>
      <ul class="pagination">
        ...
        <?php } ?>
        <li <?php if (isset($_GET['p'])) {if ($_GET['p'] == $nbrpages) {echo "class='disabled'";}} ?>><a
            href="Contact_Emp.php?f=message_envoyés&p=<?php if (isset($_GET['p'])) {if ($_GET['p'] <= $nbrpages - 1) {echo ($_GET['p'] + 1);} else {echo $nbrpages;}} ?>"><span>&raquo;</span></a>
        </li>
      </ul>
    </nav>
    <?php } ?>
...
<div id="content" class="contact">
  <div id="pager">
    <a href="Contact_Emp.php" class="btn btn-primary">Envoyer Un Message</a> <a
      href="Contact_Emp.php?f=message_envoyés&p=1" class="btn btn-warning">Messages Envoyés</a></p>
  </div>
  <hr>
  <div class="content-wrapper contcli">
    <?php if ($nb['Nb'] != 0) { ?>
    <table class="table table-hover proj2 kr1">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><a href="Lire.php?f=message_reçus&p=<?php echo $cpage . "&message=" . $r['ID'] . "&s=emp"; ?>"
              class="btn btn-primary">Lire</a></td>
        </tr>
        <?php } ?>
      </tbody>
    ...
    <nav>
      <ul class="pagination">
        <li <?php if (isset($_GET['p'])) {if ($_GET['p'] == 1) {echo "class='disabled'";}} ?>><a
            href="Contact_Emp.php?f=message_reçus&p=<?php if (isset($_GET['p'])) {if ($_GET['p'] >= 2) {echo ($_GET['p'] - 1);} else {echo "1";}} ?>"><span>&laquo;</span></a>
        </li>
        <?php for ($i = max(1, $cpage - 4); $i <= max(5, $cpage); $i++) { ?>
        <li <?php if (isset($_GET['p'])) {if ($_GET['p'] == $i) {echo "class='active'";}} ?>><a
            href="Contact_Emp.php?f=message_reçus&p=<?php echo $i; ?>"><?php echo $i; ?></a></li>
        <?php } ?>
        <li <?php if (isset($_GET['p'])) {if ($_GET['p'] == $nbrpages) {echo "class='disabled'";}} ?>><a
            href="Contact_Emp.php?f=message_reçus&p=<?php if (isset($_GET['p'])) {if ($_GET['p'] <= $nbrpages - 1) {echo ($_GET['p'] + 1);} else {echo $nbrpages;}} ?>"><span>&raquo;</span></a>
        </li>
      </ul>
    </nav>
    <?php } ?>
...
<div id="content" class="cadc">
  <p><a class="btn btn-info btn-lg" href="Contact_Emp.php?f=message_reçus&p=1">Message Reçus</a> <a
      href="Contact_Emp.php?f=message_envoyés&p=1" class="btn btn-warning btn-lg">Message Envoyés</a></p>
  <hr>
