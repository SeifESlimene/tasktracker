...
if (!empty($_POST)) {
 ...
 if (empty($nom)) {
  ...
  if (!preg_match("/^\D{1,20}$/", $nom)) {
   $e["pasvalidenom"] = "Un Nom Doit Contenir Seulement Des Alphabets Et Du 1 à 20 Chiffres Au Max!";
  } else {
   $e["g"] = "";
   $v++;
  }
 ...
 if (empty($prenom)) {
  ...
  if (!preg_match("/^\D{1,20}$/", $prenom)) {
   $e["pasvalideprenom"] = "Un Prénom Doit Contenir Seulement Des Alphabets Et Du 1 à 20 Chiffres Au Max!";
  } else {
   $e['h'] = "";
   $v++;
  }
 ...
 if ($v == 7) {
  $req_ajout = "INSERT INTO `personne` (`Nom`, `Prenom`, `datenaissance`, `email`, `mdp`, `adresse`, `cin`, `image`) VALUES ('" . $nom . "', '" . $prenom . "', '" . $datenais . "', '" . $email . "', '" . $cin . "', '" . $adresse . "', " . $cin . ", 'Client');";
  $conn->query($req_ajout);
  $ref_personne  = $conn->insert_id;
  $req_ajout_emp = "INSERT INTO `client` (`MATRICULEFISCALE`, `REFPERSONNE`) VALUES ('" . $matriculefiscal . "', " . $ref_personne . ");";
...
<div id="content">
  <div class="menubar">
    <div class="page-title">
      <h3><strong>Ajout D'un Client</strong></h3>
    </div>
  </div>
  <div class="content-wrapper">
    <div class="page-title">
      <a href="affichage_client.php?p=1" class="btn btn-primary">Liste Des Clients</a>
    </div>
    <form class="form-horizontal" method="post" action="">
      <div
        ...
        <div class="col-sm-10 col-md-8">
          <input type="text" class="form-control" name="Nom" value="<?php echo $nom; ?>" maxlength="20"
            placeholder="Ex: Slimene" />
          <span class="help-block">
            <?php if (isset($e['Nom'])) {if (!empty($e['Nom'])) {echo $e['Nom'];}} elseif (isset($e["pasvalidenom"])) {echo $e["pasvalidenom"];} ?>
          </span>
        </div>
      </div>
      <div
        ...
        <div class="col-sm-10 col-md-8">
          <input type="text" class="form-control" name="Prenom" value="<?php echo $prenom; ?>" maxlength="20"
            placeholder="Ex: Seif Eddine" />
          <span class="help-block">
            <?php if (isset($e['Prenom'])) {if (!empty($e['Prenom'])) {echo $e['Prenom'];}} elseif (isset($e["pasvalideprenom"])) {echo $e["pasvalideprenom"];} ?>
          </span>
        </div>
      </div>
      <div
        ...
        <div class="col-sm-10 col-md-8">
          ...
          <span class="help-block">
            <?php if (isset($e['datenaissance'])) {if (!empty($e['datenaissance'])) {echo $e['datenaissance'];}} elseif (isset($e["pasvalide"])) {echo $e["pasvalide"];} ?>
          </span>
        </div>
      </div>
      <div
        ...
        <div class="col-sm-10 col-md-8">
          <input type="text" class="form-control" name="email" value="<?php echo $email; ?>" maxlength="30"
            placeholder="Ex: exemple@exemple.com" />
          <span class="help-block">
            <?php if (isset($e['email'])) {if (!empty($e['email'])) {echo $e['email'];}} elseif (isset($e["pasvemail"])) {echo $e["pasvemail"];} ?>
          </span>
        </div>
      </div>
      <div
        ...
        <div class="col-sm-10 col-md-8">
          <input type="text" class="form-control" name="adresse" value="<?php echo $adresse; ?>" maxlength="30"
            placeholder="Ex: Rue De La République" />
          <span class="help-block">
            <?php if (isset($e['adresse'])) {if (!empty($e['adresse'])) {echo $e['adresse'];}} elseif (isset($e["pasvalideadresse"])) {echo $e["pasvalideadresse"];} ?>
          </span>
        </div>
      </div>
      <div
        ...
        <div class="col-sm-10 col-md-8">
          ...
          <span class="help-block">
            <?php if (isset($e['cin'])) {if (!empty($e['cin'])) {echo $e['cin'];}} ?>
            <?php if (isset($e['isnum'])) {if (!empty($e['isnum'])) {echo $e['isnum'];}} ?>
          </span>
        </div>
      </div>
      <div
        ...
        <div class="col-sm-10 col-md-8">
          <input type="text" class="form-control" name="matriculefiscale" value="<?php echo $matriculefiscal; ?>"
            maxlength="30" placeholder="Ex: 123456/Y/Z/T/000" />
          <span class="help-block">
            <?php if (isset($e['MATRICULEFISCALE'])) {if (!empty($e['MATRICULEFISCALE'])) {echo $e['MATRICULEFISCALE'];}} elseif (isset($e["pasvalideimm"])) {echo $e["pasvalideimm"];} ?>
          </span>
        </div>
      </div>
